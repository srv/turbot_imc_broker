...
class TurbotAUV : public AuvBase {
 public:
  TurbotAUV() : AuvBase(), is_plan_loaded_(false) {
    ...
    recovery_actions_ = nh_.serviceClient<safety::RecoveryAction>("/safety/recovery_action"); // the abort service requets is called as a 'recovery_action'
    client_goto_ = nh_.serviceClient<control::GotoWithYaw>("/control/goto_holonomic");
